...
'''
url = "http://10.99.99.7:8080/photoaf.jpg"  # If we use multiple IP camera mobile
...
img = cv2.resize(img, (int(rows/CONST.h),int(cols/CONST.h)))
data = QR.decodeQR(img)
...
print(int((cols/2)/CONST.w),int((rows/CONST.w)-1))
cv2.waitKey(0)
...
#    gray = cv2.dilate(gray,kernel, iterations=4)
    ...
    itr = 0
    print(startPoint[0]+startPoint[1]*(steps), max_index)
